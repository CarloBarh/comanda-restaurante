...
public function with(): array
{
    $comandas = Comanda::query()
        ->with([
            'mesa',
            'mesero',
            'detalles' => fn($q) => $q->whereHas('platillo', fn($q) => $q->where('area_id', 2)),
            'detalles.platillo',
            'detalles.tamano',
        ])
        ->whereNotIn('estado', ['finalizado', 'cerrado'])
        ->whereHas('detalles.platillo', fn($q) => $q->where('area_id', 2))
        ...
        <div class="max-w-screen-2xl mx-auto px-6 py-4 flex items-center justify-between">
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <div class="font-black text-2xl tracking-tight" style="color: #f1f5f9;">MONITOR DE COMANDAS - PIZZERÍA</div>
                </div>
            </div>
